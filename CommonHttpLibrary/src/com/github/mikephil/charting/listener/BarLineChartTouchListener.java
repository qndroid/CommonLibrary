...
public class BarLineChartTouchListener extends
		ChartTouchListener<BarLineChartBase<? extends BarLineScatterCandleData<? extends BarLineScatterCandleDataSet<? extends Entry>>>> {
	...
	public BarLineChartTouchListener(
			BarLineChartBase<? extends BarLineScatterCandleData<? extends BarLineScatterCandleDataSet<? extends Entry>>> chart,
			Matrix touchMatrix) {
		super(chart);
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (mTouchMode == DRAG) {
				...
				if (mChart.isScaleXEnabled() || mChart.isScaleYEnabled())
					...
					&& Math.abs(distance(event.getX(), mTouchStartPoint.x, event.getY(), mTouchStartPoint.y)) > 5f) {
		...
		if (event.getPointerCount() >= 2) {
			...
			if (totalDist > 10f) {
				...
				if (mTouchMode == PINCH_ZOOM) {
					...
					boolean canZoomMoreX = isZoomingOut ? mChart.getViewPortHandler().canZoomOutMoreX()
