...
/**********************************************************
 * @文件名称：DownloadProgressBar.java
 * @文件作者：renzhiqiang
 * @创建时间：2015年10月8日 下午11:00:41
 * @文件描述: ValueAnimatior的完美使用
 * @修改历史：2015年10月8日创建初始版本
 **********************************************************/
public class DownloadProgressBar extends View
{
	...
	private void drawing(Canvas canvas)
	{
		...
		case IDLE:
			...
			canvas.drawLine(mCenterX - mRadius / 2, mCenterY, mCenterX + mLengthFix, mCenterY + mRadius / 2
					+ mLengthFix, mDrawingPaint);
			canvas.drawLine(mCenterX - mLengthFix, mCenterY + mRadius / 2 + mLengthFix, mCenterX + mRadius / 2,
					mCenterY, mDrawingPaint);
			break;
		case ANIMATING_LINE_TO_DOT:
			if (!mDotToProgressAnimation.isRunning())
			{
				canvas.drawLine(mCenterX, mCenterY - mRadius / 2 + mArrowLineToDotAnimatedValue * 2 - mStrokeWidth / 2,
						mCenterX, mCenterY + mRadius / 2 - mArrowLineToDotAnimatedValue * 2 + mStrokeWidth / 2,
						mDrawingPaint);
			}
			canvas.drawLine(mCenterX - mRadius / 2 - mArrowLineToHorizontalLineAnimatedValue / 2, mCenterY, mCenterX
					+ mLengthFix, mCenterY + mRadius / 2 - mArrowLineToHorizontalLineAnimatedValue + mLengthFix,
					mDrawingPaint);
			canvas.drawLine(mCenterX - mLengthFix, mCenterY + mRadius / 2 - mArrowLineToHorizontalLineAnimatedValue
					+ mLengthFix, mCenterX + mRadius / 2 + mArrowLineToHorizontalLineAnimatedValue / 2, mCenterY,
					mDrawingPaint);
			break;
		case ANIMATING_MANUAL_PROGRESS:
			float manualProgress = ((mCenterX + mRadius / 2 + mArrowLineToHorizontalLineAnimatedValue / 2) - (mCenterX
					- mRadius / 2 - mArrowLineToHorizontalLineAnimatedValue / 2)) / 360f;
			...
			mProgressBounds.right = mCenterX - mRadius / 2 - mArrowLineToHorizontalLineAnimatedValue / 2
					+ manualProgress * mCurrentGlobalManualProgressValue;
		...
		case ANIMATING_SUCCESS:
			...
			canvas.drawLine(mCenterX - mRadius / 2 + mSuccessValue * 2 - mSuccessValue / (float) Math.sqrt(2f) / 2,
					mCenterY + mSuccessValue, mCenterX + mSuccessValue * 2 - mSuccessValue / (float) Math.sqrt(2f) / 2,
					mCenterY - mSuccessValue, mDrawingPaint);
			canvas.drawLine(mCenterX - mSuccessValue - 2 * mSuccessValue / (float) Math.sqrt(2f) / 2, mCenterY,
					mCenterX + mRadius / 2 - mSuccessValue * 2 - mSuccessValue / (float) Math.sqrt(2f) / 2, mCenterY
							+ mSuccessValue, mDrawingPaint);
			break;
		case ANIMATING_ERROR:
			mDrawingPaint.setStrokeWidth(mLineWidth);
			canvas.drawArc(mCircleBounds, 0, 360, false, mDrawingPaint);

			canvas.drawLine(mCenterX - mRadius / 2 - mRadius / 4 + mErrorValue * 2, mCenterY + mErrorValue, mCenterX
					+ mErrorValue, mCenterY - mErrorValue, mDrawingPaint);
			canvas.drawLine(mCenterX - mErrorValue, mCenterY - mErrorValue, mCenterX + mRadius / 2 + mRadius / 4
					- mErrorValue * 2, mCenterY + mErrorValue, mDrawingPaint);
			break;
		}
	...
	/**
	 * Canvas�ǴӸ�ViewgGroup������,�������WindowManager�����Canvas
	 */
	@Override
