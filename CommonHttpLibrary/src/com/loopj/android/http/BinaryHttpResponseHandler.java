...
public abstract class BinaryHttpResponseHandler extends AsyncHttpResponseHandler
{
	...
	{ RequestParams.APPLICATION_OCTET_STREAM, "image/jpeg", "image/png", "image/gif" };
	...
	public final void sendResponseMessage(HttpResponse response) throws IOException
	{
		...
		if (!foundAllowedContentType)
		{
			...
			sendFailureMessage(status.getStatusCode(), response.getAllHeaders(), null,
					new HttpResponseException(status.getStatusCode(), "Content-Type (" + contentTypeHeader.getValue()
							+ ") not allowed!"));
			return;
		}
